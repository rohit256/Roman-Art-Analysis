...
class frontalizer():
    def __init__(self,refname):
        ...
        with open(refname,'rb') as f:
            #print(f)
            ref = pkl.load(f, fix_imports=True,encoding='bytes')
            print(type(ref))
            self.refU  = ref[b'refU']
            self.A = ref[b'outA']
            self.refxy = ref[b'ref_XY']
            self.p3d = ref[b'p3d']
            self.refimg = ref[b'refimg']
    ...
    def frontalization(self,img_,facebb,p2d_):
        ...
        print(mline)
        print(int(mline))
        sumleft = np.sum(synth_front[:,0:int(mline)])
        sumright = np.sum(synth_front[:,int(mline):])
