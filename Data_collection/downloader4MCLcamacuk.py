...
def fetcher(url, ref = None):
    ...
    req = urllib.request.Request(url, None, ua)
    try:
        soc = urllib.request.urlopen(req,timeout=5)
        cont = soc.read()
        if len(cont) < 1000:
            return False,''
    except urllib.request.HTTPErrorProcessor:
        print("OK")
        return False,''
    except:
        return False,''
